$(document).ready(function() {
    ...
    $('#orders').on('keyup input', function(e) {
        var target = e.target.id
        console.log(target)
        for (var i = 0; i < checkboxes.length; i++) {
            var value = checkboxes[i].value
            console.log(value);
            if(target == 'order_quantity_'+value) {
                var new_quantity = parseInt($('#order_quantity_'+value).val());
                var price = parseInt($('#order_price_per_item_'+value).val())
                var new_total = price * new_quantity;
                $('#order_total_'+value).val(new_total)    
                getTotals()
            }
        }
    })
